import { IKeyboardEvent, IMouseEvent, StickValues } from "@babylonjs/core";


const inputControlsMap: { [key: string | number]: string } = {
    /* Keyboard Mappings */
...
const gamePadControlMap: { [key: number]: { [key: number]: string }[] } = {
    /* deviceType */
    2: [
...
function normalizeJoystickInputs(stick: StickValues) {
    const stickAngularSensitivity = 400;
    const stickMoveSensitivity = 40; // see above link
    const stickValues = new StickValues(0, 0);
    if (stick) {
...
function mapStickRotationInputToActions(stickInput: StickValues, inputMap: { [key: string]: IKeyboardEvent | IMouseEvent | number }) {
...
function mapStickTranslationInputToActions(stickInput: StickValues, inputMap: { [key: string]: IKeyboardEvent | IMouseEvent | number }) {
    // NOTE: I have no idea if this is a reasonable threshold value, test with 5.0.0-alpha23+
    if (stickInput.x >= 0.05) {
...
export default {
    ...
    mapStickRotationInputToActions,
